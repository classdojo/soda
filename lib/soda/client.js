...
Client.prototype._done = function(err) {
  if(err) {
    this.emit("error");
  }
  this._queue   = null;
...
Client.prototype.before = function(cb) {
  var n = this.queue.length;
  cb();
  var newItems = this.queue.splice(n, this.queue.length);
  this._queue = newItems.concat(this.queue);
  return this;
}


Client.prototype.next = function(next) {
  var self = this;
...
Client.prototype.nextSuccess = function(cb) {
  this.next(function(err, next) {
...
Client.prototype.nextError = function(cb) {
  this.next(function(err, next) {
    if(!err) return next();
    cb(err, next);
  })
...
Client.prototype.enqueue = function(cmd, args, fn){
  ...
  /*this.queue.push(function(){
    self.command(cmd, args, function(err, body, res){
      ...
      if (err) return self._done(err, body, res);
